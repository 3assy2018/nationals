    ...
    public function handle()
    {
        ...
        if($all == '*'){
        	...
        	foreach ($countries as $country){
						$response= Curl::to("http://battuta.medunes.net/api/region/{$country->code}/all/?key=" . config('nationals.battuta.apiKey'))->get();
        		...
        		$this->error('Please Provide countries codes');
					...
					else
					{
						foreach ($cities as $code){
							...
							$response= Curl::to("http://battuta.medunes.net/api/region/{$code}/all/?key=" . config('nationals.battuta.apiKey'))->get();
							$result_cities= json_decode($response, true);
							foreach ($result_cities as $result_city){
								$added_city = new Region($result_city);
								$country->regions()->save($added_city);
